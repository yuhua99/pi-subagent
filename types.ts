...
export function parseTasksParam(
  raw: unknown,
): { tasks: TaskSpec[] } | { error: string } | undefined {
  ...
  if (typeof value === "string") {
    try {
      ...
      return {
        error:
          "Invalid `tasks`: received a string that is not valid JSON. Provide a raw JSON array of { agent, task } objects, not a JSON-encoded string.",
      };
    }
  }
  if (!Array.isArray(value) || value.length === 0 || !value.every(isTaskSpec)) {
    return {
      error: "Invalid `tasks`: expected a non-empty array of { agent, task, cwd? } objects.",
    };
  }
  return { tasks: value };
}
