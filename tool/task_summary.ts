...
function formatToolArguments(arguments_: Record<string, unknown>): string {
	...
	for (const key in arguments_) {
		...
		const rendered = typeof value === "string" ? truncate(value, 160)
			: value === null || typeof value === "number" || typeof value === "boolean" ? String(value)
...
function formatActivityMessage(message: Message): string {
	if (message.role === "assistant") {
		...
		for (const part of message.content) {
			const entry = part.type === "text" ? `assistant: ${truncate(part.text, ACTIVITY_MESSAGE_LIMIT_CHARS)}`
				: part.type === "toolCall" ? `tool call: ${truncate(part.name, 120)}(${formatToolArguments(part.arguments)})`
				: "";
			if (!entry) continue;
			activity = truncate(`${activity}${activity ? "\n" : ""}${entry}`, ACTIVITY_MESSAGE_LIMIT_CHARS);
	...
	if (message.role === "toolResult") {
		...
		return truncate(`tool result${message.isError ? " (error)" : ""}: ${result || "(no text output)"}`, ACTIVITY_MESSAGE_LIMIT_CHARS);
	}
	const text = typeof message.content === "string" ? message.content : textContent(message.content, ACTIVITY_MESSAGE_LIMIT_CHARS);
...
export function formatActivityContext(task: string, messages: readonly Message[]): string {
	...
	const activities = messages.slice(-ACTIVITY_MESSAGE_LIMIT)
